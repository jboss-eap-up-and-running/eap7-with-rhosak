...
Creating a secret from an env file, is another way of adding a secret from link:{running-doc-url}#adding_secret[Adding a secret to OpenShift].
...
We specify that we want to provision the `cloud-server` and `microprofile-reactive-messaging-kafka` layers as mentioned in link:{running-doc-url}#trimming_server[Trimming the provisioned server] section.

In the `deploy` section, we mount a directory to via `volumes`/`volumeMounts`. As described in link:{running-doc-url}#mapping_configmap_secret_values_to_directory[Mapping entries from a ConfigMap/Secret to files in a mounted directory] files in `/etc/config/rhoas` directory will contain the entries in the `rhoas` secret we created before.
